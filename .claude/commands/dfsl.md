...
| 3.3 | [`/jira-impl $ARGUMENTS`](./jira-impl.md) | Run the full impl flow including the release-tracker bundle append (§2b). Apply the provenance prefix to commits and PR title (§5). Open the PR as `--draft` per `/jira-impl §7` (Q3 confirmed). **Dashboard UI/UX work** — `/jira-impl §3` already routes dashboard-surface changes to the [`dashboard-ui-ux`](../agents/dashboard-ui-ux.md) subagent via the `Agent` tool; DFSL inherits that delegation unchanged, **including the mandatory `record_usage.py --command dashboard-ui-ux --asset-type agent --trigger bridge` `started` + terminal bracket** so the dashboard timeline shows the subagent's activity under the DFSL run. The subagent's "could not verify" hand-back items must surface verbatim in the DFSL Disclaimer (§6) under *Items needing human verification* — autonomous mode does not get to silently drop browser-eyes gaps. |
